<script lang="ts">
    ...
    const registerSchema = yup.object({
        name: yup.string().required("Name mustn't be empty").min(4, "Name must be longer than 4 characters."),
    ...
    const {
        ...
        validate: [validateSchema(registerSchema), (values) => {
            const errors = {}
            if (domains.length < 1) {
...
<form use:form>
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col">
        <div class="mb-4">
            ...
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-800" id="name"
                   type="text"
                   name="name"
                   class:border-red-300={$errors.name !== null}
                   class:border-green-400={$touched.name === true &&
								$errors.name === null}
        ...
        <div class="mb-4">
            ...
            <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-800" id="description"
                      name="description"
        ...
        <div class="mb-6">
            ...
            <button type="button" on:click={() => {domains = [...domains, tempDomain]; tempDomain = ""; validate()}}>
                Add domain
            </button>
            {#each domains as domain, index}
                <div>
                    {domain}
                    <button type="button" on:click={() => {domains.splice(index); domains = domains; validate()}}>
                        Remove domain
                    </button>
                </div>
            {/each}
